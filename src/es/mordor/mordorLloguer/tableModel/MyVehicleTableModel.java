...
public abstract class MyVehicleTableModel<T extends Vehicle> extends MyTableModel<T> {
	...
	public MyVehicleTableModel(List<T> data) {
		super(new ArrayList<String>(Arrays.asList("Matricula", "Precio", "Modelo", "Color", "Motor", "Cilindrada",
				"Fecha Adq", "Estado", "Carnet")), data);
	...
	public Object getValueAt(int row, int col) {
		...
		case 0:
			return data.get(row).getRegistration();

		case 1:
			return data.get(row).getDayPrice();
		...
		case 4:
			return data.get(row).getEngine();

		case 5:
			return data.get(row).getDisplacement();

		case 6:
			return data.get(row).getShopDay();

		case 7:
			return data.get(row).getStatus();

		case 8:
			return data.get(row).getDrivingLicense();
